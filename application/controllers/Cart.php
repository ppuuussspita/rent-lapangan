...
class Cart extends CI_Controller
{
	...
	public function buy($id)
	{
		...
		if ($row) {
			...
			$id_trans 			= $cek_transaksi->id_trans ?? null;
	...
	public function checkout()
	{
		...
		for ($i = 0; $i < $count; $i++) {
			$data_detail[$i] = array(
				...
				'jam_mulai'     => (int)$this->input->post('jam_mulai[' . $i . ']') . ":00:00",
				'durasi'        => $this->input->post('durasi[' . $i . ']'),
				'harga_jual'    => $this->input->post('harga_jual[' . $i . ']'),
				'jam_selesai'   => (int)$this->input->post('jam_mulai[' . $i . ']') + $this->input->post('durasi[' . $i . ']') . ":00:00",
